<!-- Add Role Modal -->
<div class="modal fade" id="AgregarTemporada" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered modal-nueva-temporada">
    <div class="modal-content p-3 p-md-5">
      <button type="button" class="btn-close btn-pinned" data-bs-dismiss="modal" aria-label="Close"></button>
      <div class="modal-body">
        <div class="text-center mb-4">
          <h3 class="role-title mb-2">Nuevo periodo</h3>
          <!--<p class="text-muted">Set role permissions</p>-->
        </div>
        <!-- Add role form -->
        <form id="agregarTemporada" class="row g-3"  method="post" action="{{ route('AgregarTemporada') }}" data-token="{{ csrf_token() }}">
          @csrf
...
document.addEventListener("DOMContentLoaded", function () {
  const nuevaTemporadaModal = document.getElementById("AgregarTemporada");
  const formulario = nuevaTemporadaModal.querySelector("#agregarTemporada");
