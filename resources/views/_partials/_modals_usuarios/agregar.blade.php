    ...
    <div class="modal-content p-3 p-md-5">
      ...
      <div class="modal-body">
        <div class="text-center mb-4">
          <h3 class="role-title mb-2">Nuevo Usuario</h3>
          <!--<p class="text-muted">Set role permissions</p>-->
        </div>
        <!-- Add role form -->
        <form id="agregarUsuario" class="row g-3"  method="post" action="{{ route('AgregarUsuario') }}" data-token="{{ csrf_token() }}">
          @csrf
          ...
          <div class="col-12 mb-4">
            <label class="form-label" for="visible">Activo :</label>
...
document.addEventListener("DOMContentLoaded", function () {
  const nuevoUsuarioModal = document.getElementById("AgregarUsuario");
  nuevoUsuarioModal.addEventListener("shown.bs.modal", function () {
  nuevoUsuarioModal.querySelector("#nombre").focus();
  });
  const formulario = nuevoUsuarioModal.querySelector("#agregarUsuario");
